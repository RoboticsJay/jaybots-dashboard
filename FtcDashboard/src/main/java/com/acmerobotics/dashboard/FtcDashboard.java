...
// import
// org.firstinspires.ftc.robotcore.internal.opmode.AnnotatedOpModeClassFilter;
// import org.firstinspires.ftc.robotcore.internal.opmode.InstanceOpModeManager;
// import
// org.firstinspires.ftc.robotcore.internal.opmode.InstanceOpModeRegistrar;
...
public class FtcDashboard implements OpModeManagerImpl.Notifications {
  ...
  private static final Set<String> IGNORED_PACKAGES =
      new HashSet<>(Arrays.asList("java", "android", "com.sun", "com.vuforia",
                                  "com.google", "kotlin"));
   ...
   */
  public static void suppressOpMode() { suppressOpMode = true; }
  ...
  /**
   * Attaches a web server for accessing the dashboard through the phone (like
   * OBJ/Blocks).
   */
  @WebHandlerRegistrar
  public static void attachWebServer(Context context,
                                     WebHandlerManager manager) {
    if (instance != null) {
  ...
  /**
   * Returns the active instance instance. This should be called after {@link
   * #start(Context)}.
  ...
  private final List<TelemetryPacket> pendingTelemetry =
      new ArrayList<>(); // guarded by itself
  private volatile int telemetryTransmissionInterval =
      DEFAULT_TELEMETRY_TRANSMISSION_INTERVAL;

  private final Object configLock = new Object();
  private CustomVariable configRoot; // guarded by configLock
  private final List<String[]> varsToRemove =
      new ArrayList<>(); // guarded by configLock

  private TrajectorySequence
      uploadedPath; // = makeTrajectory(new UploadPath(new PathSegment(), new
                    // PathSegment[0]));
  ...
  private RobotStatus.OpModeStatus activeOpModeStatus =
      RobotStatus.OpModeStatus.STOPPED;
    ...
    public void run() {
      while (!Thread.currentThread().isInterrupted()) {
        try {
          ...
          for (TelemetryPacket packet :
               telemetryToSend.subList(0, telemetryToSend.size() - 1)) {
            packet.fieldOverlay().clear();
          }

          //                Log.d(TAG, "Sending Telementry Packet - " +
          //                uploadedPath);

          Canvas fieldOverlay =
              telemetryToSend.get(telemetryToSend.size() - 1).fieldOverlay();
    ...
    public void run() {
      while (!Thread.currentThread().isInterrupted()) {
        ...
        try {
          ...
          } else if ((timestamp - lastGamepadTimestamp) >
                     GAMEPAD_WATCHDOG_INTERVAL) {
  ...
  private class ListOpModesRunnable implements Runnable {
    ...
    public void run() {
      ...
      synchronized (opModeList) {
        for (OpModeMeta opModeMeta :
             RegisteredOpModes.getInstance().getOpModes()) {
      ...
      while (!Thread.currentThread().isInterrupted()) {
        try {
          ...
          source.getFrameBitmap(
              Continuation.createTrivial(new Consumer<Bitmap>() {
                @Override
                public void accept(Bitmap value) {
                  sendAll(new ReceiveImage(
                      bitmapToJpegString(value, imageQuality)));
                  latch.countDown();
                }
              }));
  ...
  private FtcDashboard() {
    ...
    gson = new GsonBuilder()
               ...
               .registerTypeAdapter(BasicVariable.class,
                                    new ConfigVariableSerializer())
               .registerTypeAdapter(BasicVariable.class,
                                    new ConfigVariableDeserializer())
               .registerTypeAdapter(CustomVariable.class,
                                    new ConfigVariableSerializer())
               .registerTypeAdapter(CustomVariable.class,
                                    new ConfigVariableDeserializer())
               .create();
  ...
  private void enable() {
    ...
    telemetryExecutorService =
        ThreadPool.newSingleThreadExecutor("dash telemetry");
    telemetryExecutorService.submit(new TelemetryUpdateRunnable());

    gamepadWatchdogExecutor =
        ThreadPool.newSingleThreadExecutor("gamepad watchdog");
  ...
  private void injectStatusView() {
    ...
    int horizontalMargin =
        (int)activity.getResources().getDimension(horizontalMarginId);
    ...
    if (connectionStatusTextView != null) {
      AppUtil.getInstance().runOnUiThread(new Runnable() {
        ...
        public void run() {
          ...
          String serverStatus =
              webServerAttached ? "server attached" : "server detached";
          ...
          connectionStatusTextView.setText("Dashboard: " + serverStatus + ", " +
                                           connStatus);
        }
      });
    }
  ...
  private WebHandler newStaticAssetHandler(final AssetManager assetManager,
                                           final String file) {
    ...
    try {
      ...
      if (list.length > 0) {
        for (String file : list) {
          addAssetWebHandlers(webHandlerManager, assetManager,
                              path + "/" + file);
        }
      } else {
  ...
  private void internalAttachEventLoop(FtcEventLoop eventLoop) {
    ...
    synchronized (opModeList) { opModeList.clear(); }
  ...
  private void internalPopulateMenu(Menu menu) {
    ...
    synchronized (enableMenuItems) { enableMenuItems.add(enable); }

    synchronized (disableMenuItems) { disableMenuItems.add(disable); }
  ...
  String toJson(Object src) { return gson.toJson(src); }

  /**
   * Queues a telemetry packet to be sent to all clients. Packets are sent in
   * batches of approximate period {@link #getTelemetryTransmissionInterval()}.
   * Clients display the most recent value received for each key, and the data
   * is cleared upon op mode init or a call to
   * {@link #clearTelemetry()}.
   *
  ...
  /**
   * Returns a {@link Telemetry} object that delegates to the telemetry methods
   * of this class. Beware that the implementation of the interface is
   * incomplete, and users should test each method they intend to use.
   */
  ...
  public void updateConfig() {
    synchronized (configLock) { sendAll(new ReceiveConfig(configRoot)); }
  }
  ...
  /**
   * Executes function in an exclusive context for config tree modification. Do
   * not leak the config tree outside the function.
   * @param function
   */
  public void withConfigRoot(Consumer<CustomVariable> function) {
    synchronized (configLock) { function.accept(configRoot); }
  }

  /**
   * Add config variable with custom provider that is automatically removed when
   * op mode ends.
   ...
   */
  public <T> void addConfigVariable(String category, String name,
  ...
  /**
   * Sends an image to the dashboard for display (MJPEG style). Note that the
   * encoding process is synchronous. Stops the active stream if running.
   ...
   * {@link #startCameraStream(CameraStreamSource, double)}
   */
  public void setImageQuality(int quality) { imageQuality = quality; }
  ...
  private RobotStatus getRobotStatus() {
    if (opModeManager == null) {
      ...
      return new RobotStatus(opModeManager.getActiveOpModeName(),
                             activeOpModeStatus);
    }
  }
  ...
  void onMessage(DashboardWebSocket socket, Message msg) {
    ...
    case UPLOAD_PATH: {
      //                try {
      //                    uploadedPath =
      //                    MakeTrajectory.fromUploadedPath((UploadPath) msg);
      //                } catch (Exception e) {
      //                    Log.e(TAG, "Exception when making trajectory to
